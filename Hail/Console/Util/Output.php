...
use Hail\Console\Util\Writer\WriterInterface;
...
class Output
{
	 ...
	 *
	 * @return \Hail\Console\Util\Output
	 */
	 ...
	 * @throws \RuntimeException if writer key doesn't exist
	 */
	public function get($writer)
	{
		if (!isset($this->writers[$writer])) {
			throw new \RuntimeException('Unknown writer [' . $writer . ']');
		}
	 ...
	 * @param mixed $writer
	 *
	 * @throws \RuntimeException For non-valid writer
	 */
	protected function handleUnknownWriter($writer)
	{
		...
		if (is_object($writer)) {
			throw new \RuntimeException('Class [' . get_class($writer) . '] must implement '
				. 'Hail\Console\Util\Writer\WriterInterface.');
		}

		// No idea, just tell them we can't resolve it
		throw new \RuntimeException('Unable to resolve writer [' . $writer . ']');
	}
