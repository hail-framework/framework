...
use Hail\Console\Argument\Manager as ArgumentManager;
use Hail\Console\Decorator\Style;
use Hail\Console\TerminalObject\Router\Router;
use Hail\Console\Util\Helper;
use Hail\Console\Util\Output;
use Hail\Console\Util\UtilFactory;
 ...
 * @method mixed confirm(string $prompt)
 * @method mixed password(string $prompt)
 * @method mixed checkboxes(string $prompt, array $options)
 * @method mixed radio(string $prompt, array $options)
...
class Console
{
	 ...
	 *
	 * @var \Hail\Console\Decorator\Style $style
	 */
	public $style;
	 ...
	 *
	 * @var \Hail\Console\TerminalObject\Router\Router $router
	 */
	protected $router;
	 ...
	 *
	 * @var \Hail\Console\Argument\Manager $arguments
	 */
	public $arguments;
	...
	public function __construct()
	{
		...
		$this->util = new UtilFactory();
		$this->arguments = new ArgumentManager();
	}

	/**
	 * Force ansi support on
	 *
	 * @return \Hail\Console\Console
	 */
	 ...
	 */
	protected function applyStyleMethods($name, $search = null): string
	{
		// Get all of the possible style attributes
		$search = $search ?: array_keys($this->style->all());

		$new_name = $this->searchForStyleMethods($name, $search);

		// While we still have a name left and we keep finding methods,
		// loop through the possibilities
		if ($new_name !== '' && $new_name !== $name) {
			return $this->applyStyleMethods($new_name, $search);
		}
	 ...
	 * @param array  $arguments
	 *
	 * @return \Hail\Console\Console|\Hail\Console\TerminalObject\Dynamic\AbstractDynamic
	 *
	...
	public function __call($method, $arguments)
	{
		...
		$name = $this->applyStyleMethods(Helper::snakeCase($method));
