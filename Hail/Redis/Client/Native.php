...
class Native extends AbstractClient
{
	...
	public function pSubscribe($patterns, $callback)
	{
		...
		while ($patternCount--) {
			if (isset($status)) {
				list($command, $pattern, $status) = $this->readReply();
			} else {
				list($command, $pattern, $status) = $this->__call('psubscribe', [$patterns]);
			}
	...
	public function subscribe($channels, $callback)
	{
		...
		while ($channelCount--) {
			if (isset($status)) {
				list($command, $channel, $status) = $this->readReply();
			} else {
				list($command, $channel, $status) = $this->__call('subscribe', [$channels]);
			}
			$this->subscribed = $status > 0;
			if (!$status) {
		...
		switch ($replyType) {
			...
			case '-':
				if ($this->isMulti || $this->usePipeline) {
					$response = false;
				} else if ($name === 'evalsha' && strpos($reply, '-NOSCRIPT') === 0) {
					$response = null;
				} else {
