...
class GoogleStorage extends AbstractAdapter
{
	...
	public function __construct(array $config)
	{
		...
		$this->storageClient = new StorageClient($config['client']);
		$this->bucket = $this->storageClient->bucket($config['bucket']);

		$this->setPathPrefix($config['prefix'] ?? null);
		$this->storageApiUri = $config['apiUri'] ?? 'https://storage.googleapis.com';
	}
	 ...
	 * @return array
	 */
	protected function upload($path, $contents, array $config)
	{
