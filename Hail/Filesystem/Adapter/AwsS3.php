...
/**
 * Class AwsS3
 * $ composer require aws/aws-sdk-php:^3.0.0
 *
...
class AwsS3 extends AbstractAdapter
{
	...
	{
		if (!isset($config['client'], $config['bucket'])) {
			throw new \InvalidArgumentException('AwsS3 config not defined!');
		}

		$this->s3Client = new S3Client($config['client']);
	...
	protected function retrievePaginatedListing(array $options)
	{
		...
		foreach ($resultPaginator as $result) {
			$listing[] = $result->get('Contents') ?: [];
			$listing[] = $result->get('CommonPrefixes') ?: [];
		}

		return call_user_func_array('array_merge', $listing);
	}
