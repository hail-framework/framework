...
class ObjectMixin
{
	...
	public static function strictGet($class, $name)
	{
		...
		$hint = self::getSuggestion(array_merge(
			array_filter($rc->getProperties(\ReflectionProperty::IS_PUBLIC), function ($p) {
				return !$p->isStatic();
			}),
			self::parseFullDoc($rc, '~^[ \t*]*@property(?:-read)?[ \t]+(?:\S+[ \t]+)??\$(\w+)~m')
		), $name);
		throw new Exception\MemberAccess("Cannot read an undeclared property $class::\$$name" . ($hint ? ", did you mean \$$hint?" : '.'));
	}
	...
	public static function strictSet($class, $name)
	{
		...
		throw new Exception\MemberAccess("Cannot write to an undeclared property $class::\$$name" . ($hint ? ", did you mean \$$hint?" : '.'));
	}
	...
	public static function strictCall($class, $method, $additionalMethods = [])
	{
		...
		throw new Exception\MemberAccess("Call to undefined method $class::$method()" . ($hint ? ", did you mean $hint()?" : '.'));
	}
	...
	public static function strictStaticCall($class, $method)
	{
		$hint = self::getSuggestion(
			array_filter((new \ReflectionClass($class))->getMethods(\ReflectionMethod::IS_PUBLIC), function ($m) {
				return $m->isStatic();
			}),
			$method
		);
		throw new Exception\MemberAccess("Call to undefined static method $class::$method()" . ($hint ? ", did you mean $hint()?" : '.'));
	}
	...
	public static function call($_this, $name, $args)
	{
		...
		if ($name === '') {
			throw new Exception\MemberAccess("Call to class '$class' method without name.");
			...
			if (is_array($_this->$name) || $_this->$name instanceof \Traversable) {
				...
				throw new Exception\UnexpectedValue("Property $class::$$name must be array or NULL, " . gettype($_this->$name) . ' given.');
			}
			...
			if (count($args) !== ($op === 'get' ? 0 : 1)) {
				throw new Exception\InvalidArgument("$class::$name() expects " . ($op === 'get' ? 'no' : '1') . ' argument, ' . count($args) . ' given.');

			} elseif ($type && $args && !self::checkType($args[0], $type)) {
				throw new Exception\InvalidArgument("Argument passed to $class::$name() must be $type, " . gettype($args[0]) . ' given.');
			}
	 ...
	 * @throws Exception\MemberAccess if the property is not defined.
	 */
	public static function & get($_this, $name)
	{
		...
		if ($name === '') {
			throw new Exception\MemberAccess("Cannot read a class '$class' property without name.");
			...
			throw new Exception\MemberAccess("Cannot read a write-only property $class::\$$name.");
	 ...
	 *
	 * @return void
	 * @throws Exception\MemberAccess if the property is not defined or is read-only
	 */
	public static function set($_this, $name, $value)
	{
		...
		if ($name === '') {
			throw new Exception\MemberAccess("Cannot write to a class '$class' property without name.");
			...
			throw new Exception\MemberAccess("Cannot write to a read-only property $class::\$$name.");
	...
	public static function remove($_this, $name)
	{
		...
		if (!self::hasProperty($class, $name)) {
			throw new Exception\MemberAccess("Cannot unset the property $class::\$$name.");
		}
	}
