...
class Parser
{
		...
		do {
			...
			if (self::preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+))?$#u', rtrim($this->currentLine), $values)) {
				...
				if (!isset($values['value']) || '' === trim($values['value'], ' ') || 0 === strpos(ltrim($values['value'], ' '), '#')) {
					$data[] = $this->parseBlock($this->getRealCurrentLineNb() + 1, $this->getNextEmbedBlock(null, true));
				} else {
				...
				try {
					...
					$i = 0;
					$key = Inline::parseScalar($values['key'], 0, null, $i, false);
				...
				if ($mergeNode) {
					...
					if (!$this->isNextLineIndented() && !$this->isNextLineUnIndentedCollection()) {
						...
						if ($allowOverwrite || !isset($data[$key])) {
							...
							throw new ParseException(sprintf('Duplicate key "%s" detected on line %d whilst parsing YAML. ', $key, $this->getRealCurrentLineNb() + 1));
						}
					} else {
						...
						if ($allowOverwrite || !isset($data[$key])) {
							...
							throw new ParseException(sprintf('Duplicate key "%s" detected on line %d whilst parsing YAML.', $key, $realCurrentLineNbKey + 1));
						}
					}
				} else {
				...
				if (isset($this->currentLine[1]) && '?' === $this->currentLine[0] && ' ' === $this->currentLine[1]) {
					throw new ParseException('Starting an unquoted string with a question mark followed by a space is not supported');
				}
	...
	private function parseBlock($offset, $yaml)
	{
	 ...
	 */
	private function parseValue($value, $context)
	{
		if (0 === strpos($value, '*')) {
