...
class Option
{
	...
	public function setValue($value)
	{
		if ($this->isBoolean() && !is_bool($value)) {
			throw new \InvalidArgumentException(sprintf('Boolean option expected for option %s, received %s value instead', $this->name, $value));
		}
		if (!$this->validate($value)) {
			throw new \InvalidArgumentException(sprintf('Invalid value, %s, for option %s', $value, $this->name));
		}
		if ($this->isFile()) {
			$file_path = $this->parseFilePath($value);
			if (empty($file_path)) {
				if ($this->file_require_exists) {
					throw new \UnexpectedValueException(sprintf('Expected %s to be a valid file, for option %s', $value, $this->name));
				}
			} else {
				$value = $file_path;
			}
