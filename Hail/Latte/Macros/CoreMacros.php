...
use Hail\Latte\CompileException;
use Hail\Latte\MacroNode;
use Hail\Latte\PhpWriter;
use Hail\Latte\MacroTokens;
use Hail\Latte\Runtime\Filters;


/**
 * Basic macros for Hail\Latte.
 *
 * - {if ?} ... {elseif ?} ... {else} ... {/if}
...
class CoreMacros extends MacroSet
{
	...
	public function macroUse(MacroNode $node, PhpWriter $writer)
	{
		...
		call_user_func(Helpers::checkCallback(array($node->tokenizer->fetchWord(), 'install')), $this->getCompiler())
			->initialize();
	}
	...
	public function macroEndForeach(MacroNode $node, PhpWriter $writer)
	{
		...
		if ($node->modifiers !== '|noiterator' && preg_match('#\W(\$iterator|include|require|get_defined_vars)\W#', $this->getCompiler()->expandTokens($node->content))) {
			$node->openingCode = '<?php $iterations = 0; foreach ($iterator = $_l->its[] = new Hail\Latte\Runtime\CachingIterator('
	...
	{
		return $writer->write('echo Filters::htmlAttributes(%node.array)');
	}
	...
	public function macroVar(MacroNode $node, PhpWriter $writer)
	{
		...
		while ($tokens->nextToken()) {
			if ($var && $tokens->isCurrent(MacroTokens::T_SYMBOL, MacroTokens::T_VARIABLE)) {
			...
			} elseif ($var === NULL && $node->name === 'default' && !$tokens->isCurrent(MacroTokens::T_WHITESPACE)) {
