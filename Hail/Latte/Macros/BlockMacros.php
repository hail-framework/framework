...
use Hail\Latte\Compiler\{
	Compiler, MacroNode, MacroTokens, PhpWriter
};
use Hail\Latte\Runtime\SnippetDriver;
		...
		if ($name == NULL) {
			...
			if ($node->name === 'snippet') {
				...
				if ($node->name === 'define') {
					...
					if (0 === strpos((string) $node->context[1], Compiler::CONTEXT_HTML_ATTRIBUTE)) {
						$node->context[1] = '';
						$node->modifiers .= '|escape';
	...
	public function macroIfset(MacroNode $node, PhpWriter $writer)
	{
		...
		while (($name = $node->tokenizer->fetchWord()) !== NULL) {
			$list[] = preg_match('~#|[\w-]+\z~A', $name)
				...
				: $writer->formatArgs(new MacroTokens($name));
		}
