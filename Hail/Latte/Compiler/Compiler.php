...
class Compiler
{
	...
	{
		if ($this->lastAttrValue === '' && $this->context && 0 === strpos($this->context, self::CONTEXT_HTML_ATTRIBUTE)) {
	...
	{
		if ($this->context === self::CONTEXT_HTML_TAG || $this->context && 0 === strpos($this->context, self::CONTEXT_HTML_ATTRIBUTE)) {
	...
	{
		if (0 === strpos($token->name, Parser::N_PREFIX)) {
	...
	public function closeMacro(string $name, string $args = NULL, string $modifiers = NULL, bool $isRightmost = FALSE, $nPrefix = NULL): MacroNode
	{
		...
		if (!$node || ($node->name !== $name && '' !== $name) || $modifiers
			|| ($args && $node->args && 0 !== strpos("$node->args ", "$args "))
			|| $nPrefix !== $node->prefix
		) {
			$name = $nPrefix
