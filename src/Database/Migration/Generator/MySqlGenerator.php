...
namespace Hail\Database\Migration\Generator;
...
class MySqlGenerator
{
     ...
     */
    public function __construct(MySqlAdapter $dba, Logger $output, $options = [])
    {
        $this->dba = $dba;
        $this->output = $output;
    ...
    public function createMigration($name, $newSchema, $oldSchema)
    {
        ...
        $output[] = '<?php';
        $output[] = '';
        $output[] = 'use Hail\Database\Migration\AbstractMigration;';
        $output[] = 'use Hail\DatabaseMigration\Adapter\MysqlAdapter;';
        $output[] = '';
        $output[] = sprintf('class %s extends AbstractCommand', $name);
        $output[] = '{';
    ...
    protected function getIndexCreate($output, $schema, $table, $indexName)
    {
        ...
        $output[] = sprintf("%s}", $this->ind2);
        $output[] = sprintf("%s\$this->table(\"%s\")->addIndex(%s, %s)->save();", $this->ind2, $table, $indexFields,
            $indexOptions);
