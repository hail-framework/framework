...
class Dumper
{
	...
	public static function emit($input, $inline = 0, $indent = 0)
	{
        ...
        if ($input instanceof \ArrayObject || $input instanceof \stdClass) {
        ...
        if ($inline <= 0 || (!\is_array($input) && $dumpObjectAsInlineMap) || empty($input)) {
			...
			$dumpAsMap = self::isHash($input);

			foreach ($input as $key => $value) {
				if ($inline >= 1 && \is_string($value) && false !== \strpos($value, "\n")) {
					$output .= \sprintf("%s%s%s |\n", $prefix, $dumpAsMap ? self::dump($key) . ':' : '-', '');
	 ...
	 */
	private static function isHash(array $value): bool
	{
        if ($value instanceof \stdClass || $value instanceof \ArrayObject) {
            return true;
        }
	 ...
	 */
	private static function requiresDoubleQuoting(string $value): bool
	{
	 ...
	 */
	private static function escapeWithDoubleQuotes(string $value): string
	{
	 ...
	 */
	private static function requiresSingleQuoting(string $value): bool
	{
	 ...
	 * @return string The quoted, escaped string
	 */
	private static function escapeWithSingleQuotes(string $value): string
	{
