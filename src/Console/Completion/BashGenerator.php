...
class BashGenerator
{
    ...
    public function renderArgumentCompletionValues(Argument $a)
    {
...
BASH;
        ...
        $funcSuffix = $this->app->getSignature();
...
");
    ...
    public function generateCompleteFunction(Buffer $buf, CommandInterface $cmd, $compPrefix)
    {
        $funcSuffix = $cmd->getSignature();
        $buf->appendLine("{$compPrefix}_complete_{$funcSuffix} ()");
        $buf->appendLine("{");
        $buf->appendLine('local comp_prefix=' . $compPrefix);
        $buf->append('
        ...
        $subcommandDescMap = [];
        $commandOptionMap = [];
        $commandOptionRequireValueMap = [];
        $commandSignMap = [];
        foreach ($subcommands as $subcommand) {
            foreach ($subcommand->aliases() as $alias) {
                $subcommandAliasMap[$alias] = $subcommand->name();
            }
            $subcommandDescMap[$subcommand->name()] = $subcommand->brief();
            $commandSignMap[$subcommand->name()] = $subcommand->getSignature();
        }
        ...
        //
        $buf->appendLine('subcommands=' . $this->array2bash($subcommandDescMap));
        $buf->appendLine('subcommand_alias=' . $this->array2bash($subcommandAliasMap));
        $buf->appendLine('subcommand_signs=' . $this->array2bash($commandSignMap));
        ...
        $buf->appendLine('options=' . $this->array2bash($commandOptionMap));

        //  options_require_value=(["--log-dir"]="__complete_directory")
        $buf->appendLine('options_require_value=' . $this->array2bash($commandOptionRequireValueMap));


        // local argument_min_length=0
        $arguments = $cmd->getArguments();
        $buf->appendLine('local argument_min_length=' . count($arguments));
    ...
    if [[ -z "$command" ]] ; then
        ...
        if ($arguments !== []) {
            ...
            foreach ($arguments as $index => $a) {
                ...
                if ($a->validValues || $a->suggestions) {
                    ...
                    if ($a->validValues) {
                        if (is_callable($a->validValues)) {
                            ...
                            $buf->appendLine('      COMPREPLY=( $(compgen -W "' . implode("\n",
                                    $values) . '" -- $cur) )');
                            $buf->appendLine('      return');
                        }
                    } elseif ($a->suggestions) {
                ...
                } elseif (in_array($a->isa, ['file', 'path', 'dir'])) {
                    $compopt = '';
                    switch ($a->isa) {
                        case 'file':
                            $compopt .= ' -A file';
...
');
    ...
    public function generateCommandCompletionRecursively(Buffer $buf, CommandInterface $command, $compPrefix)
    {
     ...
     */
    protected function array2bash(array $array)
    {
        $out = '(';
