...
class Dumper
{
    ...
    public static $objectExporters = [
        'Closure' => 'Hail\Debugger\Dumper::exportClosure',
        'SplFileInfo' => 'Hail\Debugger\Dumper::exportSplFileInfo',
        'SplObjectStorage' => 'Hail\Debugger\Dumper::exportSplObjectStorage',
        '__PHP_Incomplete_Class' => 'Hail\Debugger\Dumper::exportPhpIncompleteClass',
    ];
    ...
    private static function dumpArray(& $var, $options, $level)
    {
        ...
        if (!$options[self::DEPTH] || $level < $options[self::DEPTH]) {
            ...
            foreach ($var as $k => &$v) {
                if ($k !== $marker) {
                    $k = \is_int($k) || \preg_match('#^\w{1,50}\z#',
                        $k) ? $k : '"' . Helpers::escapeHtml(self::encodeString($k, $options[self::TRUNCATE])) . '"';
    ...
    private static function dumpObject(&$var, $options, $level)
    {
        ...
        $out = '<span class="tracy-dump-object"'
            . ($editor ? Helpers::formatHtml(
                ' title="Declared in file % on line %" data-tracy-href="%"', $rc->getFileName(), $rc->getStartLine(),
                $editor
            ) : '')
            . '>' . Helpers::escapeHtml(Helpers::getClass($var)) . '</span> <span class="tracy-dump-hash">#' . \substr(\md5(\spl_object_hash($var)),
                0, 4) . '</span>';
        ...
        if (!$options[self::DEPTH] || $level < $options[self::DEPTH] || $var instanceof \Closure) {
    ...
    private static function dumpResource(& $var, $options, $level)
    {
        ...
        if (isset(self::$resources[$type])) {
            ...
            foreach ((self::$resources[$type])($var) as $k => $v) {
                $out .= '<span class="tracy-dump-indent">   ' . \str_repeat('|  ', $level) . '</span>'
                    . '<span class="tracy-dump-key">' . Helpers::escapeHtml($k) . '</span> => ' . self::dumpVar($v,
                        $options, $level + 1);
            }
    ...
    private static function toJson(& $var, $options, $level = 0)
    {
        ...
        if (\is_array($var)) {
            ...
            foreach ($var as $k => & $v) {
                if ($k !== $marker) {
                    $k = \is_int($k) || \preg_match('#^\w{1,50}\z#', $k) ? $k : '"' . self::encodeString($k,
                            $options[self::TRUNCATE]) . '"';
        ...
        if (\is_object($var)) {
            ...
            $obj = $obj ?: [
                ...
                'editor' => empty($editor) ? null : [
                    'file' => $rc->getFileName(), 'line' => $rc->getStartLine(), 'url' => $editor,
                ],
                'level' => $level,
                'object' => $var,
            ];
