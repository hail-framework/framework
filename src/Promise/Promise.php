...
class Promise implements PromiseInterface
{
    ...
    public function then(
        ...
        if ($this->state === self::FULFILLED) {
            $fulfilled = Util::promise($this->result);

            return $fulfilled ? $fulfilled->then($onFulfilled) : $fulfilled;
        }

        // It's either cancelled or rejected, so return a rejected promise
        // and immediately invoke any callbacks.
        $rejection = Util::rejection($this->result);
    ...
    private function settle($state, $value)
    {
        ...
        if (!$value instanceof PromiseInterface) {
            ...
            Util::queue()->add(static function () use ($id, $value, $handlers) {
                foreach ($handlers as $handler) {
                    self::callHandler($id, $value, $handler);
                }
            });
        } elseif ($value instanceof self && $value->getState() === self::PENDING) {
