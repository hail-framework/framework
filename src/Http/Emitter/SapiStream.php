...
class SapiStream implements EmitterInterface
{
     ...
     */
    public static function emit(ResponseInterface $response, $maxBufferLength = 8192)
    {
        ...
        $range = static::parseContentRange($response->getHeaderLine('Content-Range'));

        if (\is_array($range) && $range[0] === 'bytes') {
            static::emitBodyRange($range, $response, $maxBufferLength);
        } else {
            static::emitBody($response, $maxBufferLength);
        }
    }
     ...
     */
    protected static function parseContentRange($header)
    {
