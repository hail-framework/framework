...
class Size
{
     ...
     * @param Point   $pivot
     */
    public function __construct(int $width = null, int $height = null, Point $pivot = null)
    {
        $this->width = $width ?? 1;
    ...
    public function fit(Size $size, $position = 'center')
    {
        ...
        $auto_height->resize($this->width, null, function (Constraint $constraint) {
        ...
        if ($auto_height->fitsInto($this)) {
            ...
            $auto_width->resize(null, $this->height, function (Constraint $constraint) {
                $constraint->aspectRatio();
            });
