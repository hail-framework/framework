...
use Hail\Filesystem\Adapter\CanOverwriteFiles;
...
class Util
{
    ...
    public static function normalizePath($path)
    {
        ...
        return self::$normalizePath[$path] = implode('/', $parts);
    }
    ...
    {
        $mimeType = MimeType::getMimeTypeByContent($content);

        if (!(null === $mimeType || \in_array($mimeType, ['application/x-empty', 'text/plain', 'text/x-asm'], true))) {
            return $mimeType;
        }

        return MimeType::getMimeType($path) ?? 'text/plain';
    }
