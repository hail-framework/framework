...
class Redis extends AbstractAdapter
{
    ...
    /**
     * @type \Hail\Redis\RedisInterface
     */
    protected $redis;
    ...
    public function has($path)
    {
        ...
        if ($this->redis->exists($this->applyPathPrefix($info['dirname']))
    ...
    public function getMimetype($path)
    {
        ...
        return isset($metadata['mimetype']) ? Util::map($metadata,
            ['mimetype' => 'mimetype']) : ['mimetype' => 'directory'];
    }
        ...
        if ($this->has($info['path']) && $this->getMetadata($info['path'])['type'] === 'dir') {
            ...
            foreach ($files as $name => $data) {
                ...
                if ($recursive === true && $data['type'] === 'dir') {
                    $result[] = $this->listContents($data['path'], true);
                }
            }

            $result = !isset($result[1]) ? $result[0] : \array_merge(...$result);
        }
    ...
    public function deleteDir($dirname)
    {
        ...
        if ($this->has($info['path']) && $this->getMetadata($info['path'])['type'] === 'dir') {
            ...
            if ($info['path'] !== '/') {
                $status = $status && ($this->redis->hDel($this->applyPathPrefix($info['dirname']),
                            $info['basename']) > 0);
            }
    ...
    public function getVisibility($path)
    {
        ...
        return isset($metadata['visibility']) ? Util::map($metadata,
    ...
    {
        if ($this->has($path)) {
            return $this->getMetadata($path)['type'] === 'dir';
        }
